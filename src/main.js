  ...
  Alpine.data("fadgramUiDocs", () => ({
    ...
    get filteredIcons() {
      return this.allIcons.filter((icon) =>
        icon.toLowerCase().includes(this.iconsSearch.toLowerCase())
      );
    },
    get pageIcons() {
      return this.filteredIcons.slice(
        (this.iconsPage - 1) * this.perPage,
        this.iconsPage * this.perPage
      );
    },
    get iconsPages() {
    ...
    alertTypes: ["default", "alert-soft", "alert-outline"],
    alertColors: ["default", "success", "info", "warning", "error"],
    alertComponent: {
      [":class"]() {
